        ...
        withTimeout(180_000) {
            suspendCancellableCoroutine<Unit> { continuation ->
                ...
                try {
                    ...
                    mediaPlayer.setOnErrorListener { _, _, _ -> complete(IOException("语音音频无法播放")); true }
    ...
    suspend fun speakSystem(text: String, language: String, rate: Float) = withContext(Dispatchers.Main.immediate) {
            ...
            suspendCancellableCoroutine<TextToSpeech> { continuation ->
                ...
                created = TextToSpeech(context) { status ->
                    ...
                    main.post {
                        ...
                        else { created.shutdown(); continuation.resumeWithException(IOException("手机语音引擎初始化失败")) }
                    }
                }
                tts = created
        ...
        try {
            ...
            if (available < TextToSpeech.LANG_AVAILABLE) throw IOException(
                if (locale.language == "zh") "手机缺少可用的中文语音。安装中文语音包，或在语音设置中选择支持中文的 Agent TTS。"
                else "手机未安装 ${locale.displayLanguage} 语音包",
            )
            if (engine.setLanguage(locale) < TextToSpeech.LANG_AVAILABLE) throw IOException("无法启用所选语言的语音")
            ...
            if (voice != null && engine.setVoice(voice) != TextToSpeech.SUCCESS) throw IOException("无法启用所选发音人")
            ...
            if (activeLanguage !in acceptedLanguages) throw IOException("手机语音服务没有匹配的${locale.displayLanguage}发音人")
                ...
                withTimeout(timeout) {
                    suspendCancellableCoroutine<Unit> { continuation ->
                        ...
                        engine.setOnUtteranceProgressListener(object : UtteranceProgressListener() {
                            ...
                            override fun onError(utteranceId: String?) { if (utteranceId == id && continuation.isActive) continuation.resumeWithException(IOException("手机语音朗读失败")) }
                        })
                        ...
                        if (engine.speak(chunk, TextToSpeech.QUEUE_FLUSH, null, id) == TextToSpeech.ERROR && continuation.isActive) {
                            continuation.resumeWithException(IOException("手机语音服务拒绝了朗读请求"))
                        }
                    }
                }
            }
